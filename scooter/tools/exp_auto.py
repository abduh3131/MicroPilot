...
# grabs the latest model output written by lane_follow
...
# pure pursuit steering that blends a near and far lookahead plus lane centering
...
# slows down in curves and when the model isnt confident
...
# main loop that reads model output and writes throttle and steering
def run(speed, rate):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="EXP AUTO")
