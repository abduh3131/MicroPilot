...
export function start(pc, dc) {
  ...
  // ─── WebSocket fallback for joystick when WebRTC unavailable ───
  let wsJoystick = null;
  function connectWsFallback() {
    const proto = location.protocol === 'https:' ? 'wss:' : 'ws:';
    wsJoystick = new WebSocket(proto + '//' + location.host + '/ws');
    wsJoystick.onopen = function() {
      console.log("WS joystick fallback connected");
      if (!controlCommandInterval) {
        controlCommandInterval = setInterval(sendJoystickCommand, 50);
      }
    };
    wsJoystick.onclose = function() {
      console.log("WS joystick fallback disconnected, reconnecting...");
      setTimeout(connectWsFallback, 2000);
    };
  }
  ...
  function sendJoystickCommand() {
    ...
    if (dc && dc.readyState === 'open') {
      dc.send(message);
    } else if (wsJoystick && wsJoystick.readyState === WebSocket.OPEN) {
      wsJoystick.send(message);
    }
  }
  ...
  // Start WS fallback immediately (WebRTC may not connect)
  connectWsFallback();
