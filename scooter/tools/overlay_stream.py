...
# draws the green driveable corridor from the model plan positions
...
# draws the four lane lines on top of the frame
...
def draw_horizon_line(img):
    ...
    while x < w:
        x2 = min(x + dash_len, w)
        cv2.line(img, (x, y), (x2, y), color, 1, cv2.LINE_AA)
        x = x2 + gap_len
    cv2.putText(img, "HORIZON", (w - 90, y - 6),
...
# draws the heads up display with mode, confidence, steering and lidar state
...
# main loop that reads camera, model output, and writes the overlay frame
def run(fps):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Overlay Stream")
