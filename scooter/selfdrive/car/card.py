...
class Car:
  ...
  def controls_update(self, CS: car.CarState, CC: car.CarControl):
    ...
    if self.sm.all_alive(['carControl']):
      ...
      # Log actuator values to CSV before CAN transmission
      try:
        csv_path = "/data/actuators.csv"
        file_exists = os.path.exists(csv_path)
        with open(csv_path, 'a', newline='') as f:
          writer = csv.writer(f)
          if not file_exists:
            writer.writerow(['timestamp_ns', 'gas', 'brake', 'steer', 'steeringAngleDeg', 'accel', 'speed'])
          ao = self.last_actuators_output
          writer.writerow([now_nanos, ao.gas, ao.brake, ao.steer, ao.steeringAngleDeg, ao.accel, ao.speed])
      except Exception:
        pass
