...
function formatLogLine(data) {
  ...
  // gokart 6 vals
  let gk = data.gokart || {};
  let steer = gk.steer !== undefined ? gk.steer.toFixed(2) : '0.00';
  let brake = gk.brake !== undefined ? gk.brake.toFixed(2) : '0.00';
  let arm = gk.arm !== undefined ? gk.arm : 0;
  let throttle = gk.throttle !== undefined ? gk.throttle.toFixed(2) : '0.00';
  let dir = gk.direction !== undefined ? gk.direction : 1;
  let ss = gk.speed_setting !== undefined ? gk.speed_setting : 3;
  let mult = gk.multiplier !== undefined ? (gk.multiplier * 100).toFixed(0) + '%' : '-';
  let serialLine = gk.serial_line || '-';
  ...
  let armText = arm ? '<span style="color:#0f0">ARM</span>' : '<span style="color:#f44">OFF</span>';
  let lidarText = lidar ? ' <span style="color:#f00;font-weight:bold">ESTOP</span>' : '';
  let brakeText = parseFloat(brake) > 0 ? '<span style="color:#f00">' + brake + '</span>' : '<span style="color:#888">' + brake + '</span>';

  // gokart: steer,brake,arm,throttle,direction,speed_setting
  return '<span style="color:#888">' + now + '</span> ' +
    '[' + armText + '] ' +
    '<span style="color:' + modeColor + '">' + mode + '</span> ' +
    'S=<span style="color:#0ff">' + steer + '</span> ' +
    'B=' + brakeText + ' ' +
    'T=<span style="color:#ff0">' + throttle + '</span> ' +
    'D=<span style="color:#aaa">' + dir + '</span> ' +
    'SS=<span style="color:#f0f">' + ss + '</span>(' + mult + ') ' +
    'conf=<span style="color:#0f0">' + conf + '</span> ' +
    '#' + frm +
    lidarText +
    ' <span style="color:#555">| ' + serialLine + '</span>';
}
...
// speed setting btns for gokart
$(document).ready(function() {
  $('.speed-btn').click(function() {
    var ss = $(this).data('ss');
    $('.speed-btn').removeClass('active btn-light').addClass('btn-outline-light');
    $(this).removeClass('btn-outline-light').addClass('active btn-light');
    var pct = Math.round((ss + 1) / 6 * 100);
    $('#speed-status').text('Speed: ' + ss + ' (' + pct + '%)');
    fetch('/speed_setting', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({speed_setting: ss})
    });
  });
});
