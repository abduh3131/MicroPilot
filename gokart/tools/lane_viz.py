#!/usr/bin/env python3
"""offline tool that runs the model on a video file and renders lanes on top"""
...
# preprocesses a bgr frame into the supercombo input planes
...
# pulls the four lane lines out of the raw model output
...
# picks the highest probability planned path
...
# draws the planned path corridor and a center line
...
# tensorrt gpu inference wrapper
class TRTInference:
...
# walks a video file and renders the model overlay
...
def main():
    parser = argparse.ArgumentParser(description="Lane Viz, openpilot model overlay")
    ...
    parser.add_argument("--rhd", action="store_true", help="Right-hand drive (left-hand traffic, e.g. Japan/UK)")
