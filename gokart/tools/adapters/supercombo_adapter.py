"""wraps openpilot supercombo model, does yuv preprocess, runs inference, and parses the output into lanes and path"""
...
# tensorrt gpu inference that loads the engine and allocates buffers
class TRTInference:
...
# onnx cpu fallback for machines without tensorrt
class ORTInference:
...
# runs supercombo and parses lanes and path for steering
class SupercomboAdapter(BaseModelAdapter):
    ...
    def _preprocess_frame(self, bgr_img):
        ...
        # critical the y plane must be subsampled not quadrant cropped to match loadyuv kernel
        y_0 = y_plane[0::2, 0::2]
        y_1 = y_plane[1::2, 0::2]
        y_2 = y_plane[0::2, 1::2]
        y_3 = y_plane[1::2, 1::2]
        ...
        # keep raw 0 to 255 values, do not divide by 255 because the model expects raw pixels
