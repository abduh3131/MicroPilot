...
class SelfdriveD:
  def __init__(self, CP=None):
    ...
    self.calibrated_pose = None  # type: Pose or None
    ...
    if CS.canValid:
      ...
      if self.CP.notCar:
        ...
        if self.sm.frame > int(5. / DT_CTRL) and self.initialized:
          # Check web UI engagement request (file-based toggle)
          try:
            with open('/tmp/engage', 'r') as _ef:
              _engage_requested = _ef.read().strip() == '1'
          except FileNotFoundError:
            _engage_requested = False
          if _engage_requested:
